...
- `liveness_service.py`：已实现 MiniFASNet ONNX 纹理活体检测引擎，含自适应 gamma 暗光校正与模型自动降级。
...
- [x] 动作活体检测真实接入（MediaPipe Face Mesh 双动作序列：眨眼+张嘴）
- [x] 纹理活体检测真实接入（MiniFASNet ONNX 128x128 RGB，含自适应暗光 gamma 校正）
- [x] 活体结果参与后端安全判定（活体检测失败则拒绝考勤）
...
- [x] 抗照片攻击能力（后端 MiniFASNet 纹理活体检测区分真实人脸与打印照片）
- [x] 抗视频重放攻击能力（前端双动作序列时序验证 + 后端纹理检测）
- [x] 时序一致性与伪造检测能力（前端 MediaPipe EAR/MAR 动作时序验证）
...
当前版本属于”课程设计增强版交付版本”，特点如下：

- 前后端结构完整；
- 主要业务接口齐全；
- 页面与路由已可使用；
- 支持学生管理、考勤、合照、统计、导出、示例导入；
- **前端动作活体检测**（MediaPipe Face Mesh 双动作序列：眨眼+张嘴）
- **后端纹理活体检测**（MiniFASNet ONNX，含自适应暗光校正）
- **真实人脸识别**（InsightFace buffalo_l，RetinaFace 检测 + ArcFace 512 维特征）
- 可启动、可调试、可演示、可继续扩展。

### 4.2 当前版本局限

- 情绪识别已接入 DeepFace 模型但未完成专项精度评测；
- 合照识别多人脸检测效果受摄像头质量与距离影响；
- 活体检测在极端暗光/逆光环境下可能需要多次尝试；
- MiniFASNet 模型对手机屏翻拍攻击防御能力有限（依赖前端动作检测作为补充防线）。

---

## 5. 结论

截至当前阶段，课程设计核心功能已基本完成：

1. [x] 真实人脸识别模型接入（InsightFace buffalo_l：RetinaFace + ArcFace）
2. [x] 活体检测（前端 MediaPipe 动作活体 + 后端 MiniFASNet 纹理活体）
3. [x] 抗攻击能力（双动作序列时序验证 + 纹理伪影检测）

情绪分析功能已引入 DeepFace 表情识别模型，可作为课程设计”情绪分析”核心功能的实现基础。

这意味着当前项目已经具备：

- 完整考勤流程演示能力（签到 → 动作验证 → 活体检测 → 人脸识别 → 考勤记录）；
- 源码提交能力；
- 部署与运行说明；
- 后续继续优化精度和扩展功能的工程基础。
